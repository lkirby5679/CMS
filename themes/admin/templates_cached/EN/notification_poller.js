...
{var url='http://csm.transworldinteractive.net/data/notifications.php?type=mark_all_read';if(typeof window.max_notifications_to_show!='undefined')url+='&max='+window.max_notifications_to_show;url+='&time_barrier='+window.encodeURIComponent(window.notifications_time_barrier);url+='&forced_update=1';url+=keep_stub();do_ajax_request(url,window._poll_for_notifications);_toggle_messaging_box(event,'web_notifications',true);return false;}
...
var url='http://csm.transworldinteractive.net/data/notifications.php?type=poller';if(typeof window.max_notifications_to_show!='undefined')url+='&max='+window.max_notifications_to_show;url+='&time_barrier='+window.encodeURIComponent(window.notifications_time_barrier);if(forced_update)url+='&forced_update=1';url+=keep_stub();do_ajax_request(url,window._poll_for_notifications);}
...
{var go_func=function(){var sound_url='data/sounds/message_received.mp3';var base_url=((sound_url.indexOf('data_custom')==-1)&&(sound_url.indexOf('uploads/')==-1))?'':'http://csm.transworldinteractive.net';var sound_object=window.soundManager.createSound({url:base_url+'/'+sound_url});if(sound_object&&document.hasFocus())sound_object.play();};if(!window.soundManager.setupOptions.url)
